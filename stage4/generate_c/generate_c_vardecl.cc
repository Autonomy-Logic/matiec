...
void *visit(array_initial_elements_list_c *symbol) {
  if (wanted_varformat == localinit_vf || wanted_varformat == constructorinit_vf) {
    ...
    // FB arrays are initialized via loop-based FB_TYPE_init__() calls (generated by
    // init_fb_array() or by global_var_list_c), not via init_array_values() which
    // would crash because array_default_value is NULL for function blocks.
    array_specification_c *arr_spec = dynamic_cast<array_specification_c*>(this->current_var_type_symbol);
    if (arr_spec == NULL) {
      array_spec_init_c *asi = dynamic_cast<array_spec_init_c*>(this->current_var_type_symbol);
      if (asi != NULL)
        arr_spec = dynamic_cast<array_specification_c*>(asi->array_specification);
    }
    if (arr_spec != NULL && get_datatype_info_c::is_function_block(arr_spec->non_generic_type_name))
      return NULL;
...
void *visit(global_var_list_c *symbol) {
  ...
  switch (wanted_varformat) {
    ...
    case constructorinit_vf:
    {
      // Check for array-of-FB: the type is an array whose base element is a function block.
      // These need loop-based initialization calling FB_TYPE_init__() on each element,
      // rather than the __INIT_GLOBAL macro which doesn't work for FB arrays.
      bool is_array_of_fb = false;
      array_specification_c *array_spec = NULL;
      if (!is_fb) {
        array_spec = dynamic_cast<array_specification_c*>(this->current_var_type_symbol);
        if (array_spec == NULL) {
          array_spec_init_c *asi = dynamic_cast<array_spec_init_c*>(this->current_var_type_symbol);
          if (asi != NULL)
            array_spec = dynamic_cast<array_specification_c*>(asi->array_specification);
        }
        if (array_spec != NULL)
          is_array_of_fb = get_datatype_info_c::is_function_block(array_spec->non_generic_type_name);
      }

      if (is_array_of_fb) {
        // Generate loop-based initialization for global arrays of function blocks.
        // For ARRAY [0..9] OF FB_TYPE, generates:
        //   for (int __d0 = 0; __d0 < 10; __d0++) {
        //     FB_TYPE_init__(&(*GLOBAL__VARNAME).value.table[__d0], retain);
        //   }
        list_c *subrange_list = dynamic_cast<list_c*>(array_spec->array_subrange_list);
        if (subrange_list == NULL) ERROR;

        for(int i = 0; i < list->n; i++) {
          s4o.print(nv->get());

          // Generate nested for-loops, one per array dimension
          for (int d = 0; d < subrange_list->n; d++) {
            subrange_c *subrange = dynamic_cast<subrange_c*>(subrange_list->get_element(d));
            if (subrange == NULL) ERROR;
            char dim_str[128];
            snprintf(dim_str, sizeof(dim_str), "for (int __d%d = 0; __d%d < %llu; __d%d++) {\n",
                     d, d, subrange->dimension, d);
            s4o.print(dim_str);
            s4o.indent_right();
            s4o.print(s4o.indent_spaces);
          }

          // Generate: FB_TYPE_init__(&(*GLOBAL__VARNAME).value.table[__d0][__d1]..., retain);
          ...
          for (int d = 0; d < subrange_list->n; d++) {
            char subscript[32];
            snprintf(subscript, sizeof(subscript), "[__d%d]", d);
            s4o.print(subscript);
          }
          print_retain();
          s4o.print(");");

          // Close nested for-loops
          for (int d = subrange_list->n; d > 0; d--) {
            s4o.print("\n");
            s4o.indent_left();
            s4o.print(s4o.indent_spaces);
            s4o.print("}");
      ...
      else if (this->current_var_init_symbol != NULL || is_fb) {
        for(int i = 0; i < list->n; i++) {
          s4o.print(nv->get());
